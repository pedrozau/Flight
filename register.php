...
if (isset($_POST['submit'])) {
	...
	if ($password == $cpassword) {
		...
		if (!$result->num_rows > 0) {
			...
			$result = mysqli_query($conn, $sql);
			if ($result) {
				echo "<script>alert('Registro concluído')</script>";
				$username = "";
				$email = "";
				$_POST['password'] = "";
				$_POST['cpassword'] = "";
			} else {
				echo "<script>alert('Woops! Algo correu mal.')</script>";
			}
		} else {
		...
		echo "<script>alert('As palavras-passe não correspondem.')</script>";
	}
}
...
<head>
	...
	<title>Registre-se aqui! - CAEN</title>
</head>
<body>
	<div class="container">
